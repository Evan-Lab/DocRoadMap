...
// fait un faux ("mock") axios
jest.mock('axios');
const mockedAxios = axios as jest.Mocked<typeof axios>;

// mockup du chrome storage API
...
describe('RoadmapView', () => {
  ...
  it('shows error if token is missing', async () => {
    mockGet.mockImplementation((_key, cb) => cb({})); // No token
    render(<RoadmapView />);
  ...
  it('shows error if axios fails', async () => {
    ...
    render(<RoadmapView />);
    expect(await screen.findByText(/Impossible de récupérer les roadmaps/i)).toBeInTheDocument();
  });
  ...
  it('renders default image for unknown card name', async () => {
    ...
    expect(img).toHaveAttribute('src', expect.stringContaining('docroadmap.png'));
  });
});
